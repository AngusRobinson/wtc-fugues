...
export function AnalyticalMap({study,onSeek,onPassage}:{study:FugueData;onSeek:(q:number,id?:string|null,scroll?:boolean)=>void;onPassage:(start:number,end:number)=>void}){
 ...
 const tonalRowStep=44;
 const mapHeight=rowBottom+164+Math.max(0,...tonalLayout.map(t=>t.row))*tonalRowStep;
 ...
 <div className="analysis-map-scroll"><svg className="analysis-map-svg" viewBox={`0 0 1200 ${mapHeight}`} role="group" aria-label={`${formalGroups.length} principal sections, thematic entries in ${voices.length} voices, and modulation markers`}>
 ...
 {tonalLayout.map(t=>{const y=tonalY+29+t.row*tonalRowStep;return <g key={t.id} className={'tonal-marker '+(toneId===t.id?'selected':'')} role="button" tabIndex={0} aria-label={'Bar '+t.bar+': '+t.key+', '+t.roman+', '+t.type} onClick={()=>chooseTone(t.id,t.q)} onKeyDown={activate(()=>chooseTone(t.id,t.q))}><title>{'bar '+t.bar+' · '+t.key+' ('+t.roman+') · '+t.evidence}</title><line x1={t.px} x2={t.px} y1={rowBottom} y2={tonalY+3} stroke="#6d7888"/><path d={'M '+t.px+' '+(tonalY+5)+' L '+t.labelX+' '+(y-13)} stroke="#abb3bd" fill="none"/>{t.type==='cadence'||t.type==='close'?<path d={'M '+t.px+' '+(tonalY-5)+' l 5 5 l -5 5 l -5 -5 Z'} fill="#35465e"/>:<circle cx={t.px} cy={tonalY} r={toneId===t.id?4:3} fill={t.type==='entry'?'#fff':'#35465e'} stroke="#35465e"/>}<rect x={t.labelX-29} y={y-12} width="58" height="39" rx="2" fill={toneId===t.id?'#e8edf4':'#fff'}/><text x={t.labelX} y={y+2} textAnchor="middle">{t.bar} {t.short}</text><text className="tonal-roman" x={t.labelX} y={y+19} textAnchor="middle">{t.roman}</text></g>;})}
 </g>
 </svg></div>
 ...
 {tone&&<div className="tonal-evidence" aria-live="polite"><p><strong>Bar {tone.bar} · {tone.key} ({tone.roman}).</strong> {tone.evidence}</p><Button variant="ghost" size="sm" onClick={()=>onSeek(tone.q)}>Score ↗</Button></div>}
 <p className="map-note">{study.notes.map} Numerals relative to {study.metadata.key}.</p>
