...
raw=(root/'sources/bwv852.krn').read_text()
...
assert clocks==[148.0]*len(clocks) and bar==37
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':852})
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in sorted(staff.findall('.//m:note',ns),key=lambda n:by_id[n.get(ID)]['start']):
   ...
   alt=codes[sign] if sign else (carried[(staff.get('n'),key)] if e['tied'] else state.get(key,{'b': -1, 'e': -1, 'a': -1}.get(pname,0)))
...
(root/'sources/bwv852-3-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':148.0},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 37 bars; every pitch/onset agrees with Verovio.')
