...
source=ET.parse(R/'sources/bwv852-3-voices.mei').getroot()
...
ranges=[(a,a+3) for a in range(1,33,4)]+[(33,37)]
def serial(root):
...
LOW_ALTO=set(range(13,18))|{29,30,36,37}
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'3f','mode':'major','pname':'e'});ET.SubElement(st,M+'meterSig',{'count':'4','unit':'4'})
  ...
  for sn in [1,2]:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2,'x':2}[target.get('accid.ges')];before=state.get(key,-1 if key[0] in 'bea' else 0)
...
def complete(is_keyboard):
 ...
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2300 if is_keyboard else 2500,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':852,'mnumInterval':1,'spacingStaff':12 if is_keyboard else 11})
...
def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,2,0,0,6.25),('cs1',3,4,0,8.5,14.5)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':1900,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':852,'mnumInterval':1})
