...
source=ET.parse(R/'sources/bwv873-three-voices.mei').getroot()
...
colours={'subject':'#244f91','second':'#a45112','motif':'#637878'}
ranges=[(a,min(a+3,71)) for a in range(1,72,4)]
# Keep the low middle voice readable without changing its analytical identity.
low_alto=set(range(10,16))|set(range(30,36))|{42,43,53,54}|set(range(61,66))
def keyboard_staff(v,bar):return 2 if v==2 or (v==1 and bar in low_alto) else 1
def serial(root):
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'4s','mode':'minor','pname':'c','accid':'s'});ET.SubElement(st,M+'meterSig',{'count':'12','unit':'16'})
 for m in root.findall('.//m:measure',NS):
  old=m.findall(M+'staff');assert len(old)==3
  for st in old:m.remove(st)
  bar=int(m.get('n'))
  distribution=[(1,[0]),(2,[1,2])] if bar in low_alto else [(1,[0,1]),(2,[2])]
  for sn,vs in distribution:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2}[target.get('accid.ges')];before=state.get(key,1 if key[0] in 'fcgd' else 0)
  ...
  for c in list(m):
   ...
   if c.tag==M+'tie':
    v=notes[c.get('startid')[1:]]['voice'];c.set('curvedir','below' if (v==1 and bar not in low_alto) or (v==2 and bar in low_alto) else 'above')
 return root
...
def complete(is_keyboard):
 ...
 if is_keyboard:root=keyboard(root)
  ...
  previous=False
  for m in root.findall('.//m:measure',NS):
   low=int(m.get('n')) in low_alto
   if low!=previous:
    layer=m.find('m:staff[@n="2"]/m:layer',NS)
    layer.insert(0,ET.Element(M+'clef',{'shape':'F' if low else 'G','line':'4' if low else '2',ID:'alto-clef-'+m.get('n')}))
   previous=low
 ...
 for start,end in ranges:
  ...
  for ann in anns:
   ...
   if ann['start']<(start-1)*3 and ann['end']-(start-1)*3<=1:continue
   first=ns[0];v=ann['voice'];m=sec.find(f'm:measure[@n="{first["bar"]}"]',NS)
   staff=keyboard_staff(v,first['bar'])
   d=ET.SubElement(m,M+'dir',{ID:f'mark-{ann["id"]}-{start}','staff':str(staff if is_keyboard else v+1),'startid':'#'+first['id'],'place':'below' if is_keyboard and ((v==1 and first['bar'] not in low_alto) or (v==2 and first['bar'] in low_alto)) else 'above','color':colours[ann['kind']]})
   text=('SAB'[v]+': ' if is_keyboard else '')+ann['label']+(' >' if ann['start']<(start-1)*3 else '')
 ...
 assert len(root.findall('.//m:tie',NS))==len(source.findall('.//m:tie',NS))==90
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2300 if is_keyboard else 2500,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':873,'mnumInterval':1,'spacingStaff':14 if is_keyboard else 11})
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/84)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==1443
 ...
 print(mode,': 18 systems, all 1443 notes and 90 ties preserved.',flush=True)

def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,3,2,0,6.75),('second',35,37,0,102,110.25),('motif',13,14,0,37.5,40.5)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':1900,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':874,'mnumInterval':1})
