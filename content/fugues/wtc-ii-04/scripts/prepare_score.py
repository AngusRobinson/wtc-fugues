...
raw=(root/'sources/bwv873.krn').read_text()
...
for line_no,line in enumerate(raw.splitlines(),1):
 ...
 if line.startswith('='):
  ...
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*3]*3,(bar,clocks)
  continue
...
assert clocks==[213]*3 and bar==71
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':873})
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/84)<2,(e,values)
(root/'sources/bwv873-three-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':213},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 71 bars; every pitch/onset agrees with Verovio.')
