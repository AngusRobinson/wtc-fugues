...
for mode,suffix,description in [('open','annotated','Three staves'),('keyboard','keyboard','Two staves')]:
 ...
 for page,batch in enumerate(batches):
  ...
  if page==0:
   ...
   c.setFont('Helvetica',9);c.drawRightString(W-left,H-33,'J. S. BACH / BWV 873')
   c.setFont('Helvetica',8);c.drawString(left,H-50,'WTC II, no. 4 / Annotated score / '+description+' / 12/16')
   for x,lab,col in [(left,'S1 / A1 first subject / real answer','#244f91'),(left+205,'S2 second subject','#a45112'),(left+345,'Derived figures','#637878')]:
...
for name,source,indices,title,edition,url in extracts:
 ...
 for j,i in enumerate(indices):
  ...
  for y,line in zip([47,35,23],[title,edition,'BWV 873. Complete original pages; adjoining commentary is retained.']):
 ...
 writer.add_metadata({'/Title':f'{name.title()} - Analysis of Bach Fugue in C-sharp minor, BWV 873','/Author':title.split(':')[0],'/Subject':edition+' Public-domain source extract. '+url})
 dest=R/f'pdf/{name}-bwv873-analysis.pdf';writer.write(dest);print(dest.name,len(indices),'pages')
