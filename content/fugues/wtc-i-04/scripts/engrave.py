...
source=ET.parse(R/'sources/bwv849-five-voices.mei').getroot()
...
colours={'subject':'#244f91','subject2':'#a45112','subject3':'#7d3b7b','motif':'#637878'}
ranges=[(a,min(a+4,115)) for a in range(1,116,5)]
low_alto=set(range(58,72))
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'4s','mode':'minor','pname':'c'});ET.SubElement(st,M+'meterSig',{'count':'2','unit':'2'})
...
def complete(is_keyboard):
 ...
 assert len(root.findall('.//m:tie',NS))==len(source.findall('.//m:tie',NS))==103
 tidy_score(root,notes,is_keyboard,R.name)
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2500 if is_keyboard else 2700,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':849,'mnumInterval':1,'spacingStaff':16 if is_keyboard else 10})
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/107)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==1429
 ...
 print(mode,': 23 systems, all 1429 notes and 103 ties preserved.',flush=True)

def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,4,4,0,13),('subject2',35,38,0,137,148.5),('subject3',49,51,2,193,201)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2200 if kind=='subject' else 1800,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':850,'mnumInterval':1})
