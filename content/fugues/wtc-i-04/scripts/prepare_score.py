"""Parse the credited five-spine score, preserving closing chords and ties."""
...
raw=(root/'sources/bwv849.krn').read_text()
events=[];clocks=[0.,0.,0.,0.,0.];bar=0
for line_no,line in enumerate(raw.splitlines(),1):
 if not line or line[0] in '! *':continue
 if line.startswith('='):
  match=re.match(r'=(\d+)',line)
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*4]*5,(bar,clocks)
  continue
 tokens=line.split('\t');assert len(tokens)==5
 for spine,token in enumerate(tokens):
  ...
  for j,part in enumerate(chord,1):
   ...
   events.append(dict(id=f'note-L{line_no}F{spine+1}'+(f'S{j}' if len(chord)>1 else ''),voice=4-spine,start=start,duration=duration,pitch=pitch,bar=bar,name=letter.upper()+'♭'*acc.count('-')+'♯'*acc.count('#')+str(octave),diatonic=octave*7+'cdefgab'.index(letter),token=part,tied=('_' in part or ']' in part)))
assert clocks==[460]*5 and bar==115
...
# The source keeps S/A on one stave; the analytical score separates all five.
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':460},ensure_ascii=False,separators=(',',':')))
print('Converting Humdrum',flush=True)
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':849})
assert tk.loadData(raw.replace('*staff2\t*staff2\t*staff1\t*staff1\t*staff1','*staff5\t*staff4\t*staff3\t*staff2\t*staff1'))
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in staff.findall('.//m:note',ns):
   ...
   alt=codes[sign] if sign else (sustained[(e['voice'],key)] if e['tied'] else state.get(key,1 if pname in 'fcgd' else 0))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/107)<2,(e,values)
(root/'sources/bwv849-five-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':460},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 115 bars; every pitch/onset agrees with Verovio.')
