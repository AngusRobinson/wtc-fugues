...
for mode,suffix,description in [('open','annotated','Three staves'),('keyboard','keyboard','Two staves')]:
 ...
 batches=[systems[:5],systems[5:10],systems[10:14],systems[14:18],systems[18:]]
 dest=R/f'pdf/bach-g-major-fugue-{suffix}.pdf';c=canvas.Canvas(str(dest),pagesize=A4,pageCompression=1,invariant=1)
 c.setTitle(f'Bach - Fugue in G major, BWV 860 - {description}');c.setAuthor('J. S. Bach; analysis with reference to Keller, Prout and Tovey');c.setCreator('WTC fugue analyses');c.setSubject('Real answer, inverted counter-exposition, stretti and episodic triple counterpoint.')
 for page,batch in enumerate(batches):
  c.setFillColor(HexColor('#202b39'))
  if page==0:
   c.setFont('Times-Roman',20);c.drawString(left,H-34,'Fugue in G major')
   c.setFont('Helvetica',9);c.drawRightString(W-left,H-33,'J. S. BACH / BWV 860')
   c.setFont('Helvetica',8);c.drawString(left,H-50,'WTC I, no. 15 / Annotated score / '+description+' / 6/8')
   for x,lab,col in [(left,'S subject / A real answer','#244f91'),(left+205,'CS countersubject','#a45112'),(left+345,'Episode figures','#637878')]:
   ...
   c.drawString(left,H-81,'i inversion; * adapted / incomplete; > continuation; p rising third; q slower line; r semiquaver figure; d scale')
   c.drawString(left,H-93,'The closing additional parts share the S and B staves.')
   y=H-105
  else:
   y=H-28
...
extracts=[
 ('prout',args.prout,[41,42],"Ebenezer Prout: Analysis of J. S. Bach's Forty-Eight Fugues",'Ed. Louis B. Prout. London: Edwin Ashdown, 1910. Printed pp. 38-39.',"https://waltercosand.com/CosandScores/Composers%20L-P/Composers_P/Prout%2C%20Ebenezer/Prout-Analysis_of_Bach%27s_48_Fugues.pdf"),
 ('tovey',args.tovey,[115,116],"Donald Francis Tovey: The Well-Tempered Clavier, Book I",'Associated Board, 1924. Printed pp. 96-97. Fugue XV commentary.', 'https://s9.imslp.org/files/imglnks/usimg/4/4f/IMSLP911003-PMLP05948-Bach_-_48_Preludes_and_Fugues_%28Tovey%29_-_Book_I.pdf')]
for name,source,indices,title,edition,url in extracts:
 ...
 for j,i in enumerate(indices):
  ...
  for y,line in zip([47,35,23],[title,edition,'BWV 860. Complete original pages; adjoining commentary is retained.']):
 ...
 writer.add_metadata({'/Title':f'{name.title()} - Analysis of Bach Fugue in G major, BWV 860','/Author':title.split(':')[0],'/Subject':edition+' Public-domain source extract. '+url})
 dest=R/f'pdf/{name}-bwv860-analysis.pdf';writer.write(dest);print(dest.name,len(indices),'pages')
