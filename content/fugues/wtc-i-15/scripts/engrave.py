...
source=ET.parse(R/'sources/bwv860-three-voices.mei').getroot()
...
ranges=[(a,min(a+3,86)) for a in range(1,87,4)]
low_alto=set(range(25,28))|set(range(60,69))
def keyboard_staff(v,bar):return 1 if v==0 or (v==1 and bar not in low_alto) else 2
def serial(root):
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'1s','mode':'major','pname':'g'});ET.SubElement(st,M+'meterSig',{'count':'6','unit':'8'})
  ...
  for sn,vs in distribution:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2}[target.get('accid.ges')];before=state.get(key,1 if key[0] in 'f' else 0)
...
def complete(is_keyboard):
 ...
 if is_keyboard:root=keyboard(root)
  ...
  previous=False
  for m in root.findall('.//m:measure',NS):
   high=int(m.get('n')) in low_alto
   if high!=previous:
    m.find('m:staff[@n="2"]/m:layer',NS).insert(0,ET.Element(M+'clef',{'shape':'F' if high else 'G','line':'4' if high else '2',ID:'alto-clef-'+m.get('n')}))
   previous=high
 ...
 assert len(root.findall('.//m:tie',NS))==len(source.findall('.//m:tie',NS))==63
 tidy_score(root,notes,is_keyboard,R.name)
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2500 if is_keyboard else 2700,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':860,'mnumInterval':1,'minLastJustification':0,'spacingStaff':16 if is_keyboard else 10})
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/102)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==1759
 ...
 print(mode,': 22 systems, all 1759 notes and 63 ties preserved.',flush=True)

def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,5,0,0,12.5),('cs1',6,9,0,16.5,24.25),('motif',17,19,None,48,57)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2200 if kind=='subject' else 1800,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':861,'mnumInterval':1})
