"""Parse the credited 4-spine score, preserving closing chords and ties."""
...
raw=(root/'sources/bwv846.krn').read_text()
# Retain the primary (sic) reading of the three editorial ossia passages.
# Keep line numbers stable for reproducible note identifiers.
lines=[];omit=None
for line in raw.splitlines():
 fields=line.split('\t')
 if '*^' in fields:
  assert omit is None
  omit=fields.index('*^')+1;fields[omit-1]='*';lines.append('\t'.join(fields));continue
 if omit is not None and len(fields)>1:
  if '*v' in fields:
   fields.pop(omit);fields[omit-1]='*';omit=None
  else:fields.pop(omit)
  if all(f=='.' for f in fields):line='!! alternate-reading onset'
  else:line='\t'.join(fields)
 lines.append(line)
raw='\n'.join(lines)+'\n'
events=[];clocks=[0.0, 0.0, 0.0, 0.0];bar=0
for line_no,line in enumerate(raw.splitlines(),1):
 if not line or line[0] in '! *':continue
 if line.startswith('='):
  match=re.match(r'=(\d+)',line)
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*4]*4,(bar,clocks)
  continue
 tokens=line.split('\t');assert len(tokens)==4
 for spine,token in enumerate(tokens):
  ...
  for j,part in enumerate(chord,1):
   ...
   events.append(dict(id=f'note-L{line_no}F{spine+1}'+(f'S{j}' if len(chord)>1 else ''),voice=3-spine,start=start,duration=duration,pitch=pitch,bar=bar,name=letter.upper()+'♭'*acc.count('-')+'♯'*acc.count('#')+str(octave),diatonic=octave*7+'cdefgab'.index(letter),token=part,tied=('_' in part or ']' in part)))
assert clocks==[108]*4 and bar==27
...
# Separate the source staves into individual voice parts.
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':108},ensure_ascii=False,separators=(',',':')))
print('Converting Humdrum',flush=True)
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':846})
assert tk.loadData(re.sub(r'^\*staff.*$', '\t'.join('*staff'+str(i) for i in range(4,0,-1)),raw,flags=re.M))
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in staff.findall('.//m:note',ns):
   ...
   alt=codes[sign] if sign else (sustained[(e['voice'],key)] if e['tied'] else state.get(key,0 if pname in '' else 0))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/62)<2,(e,values)
(root/'sources/bwv846-4-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':108},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 27 bars; every pitch/onset agrees with Verovio.')
