...
source=ET.parse(R/'sources/bwv846-4-voices.mei').getroot()
...
ranges=[(a,min(a+3,27)) for a in range(1,28,4)]
low_alto=set()
def keyboard_staff(v,bar):return 1 if v in [0,1] else 2
def serial(root):
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'0','mode':'major','pname':'c'});ET.SubElement(st,M+'meterSig',{'count':'4','unit':'4'})
 for m in root.findall('.//m:measure',NS):
  old=m.findall(M+'staff');assert len(old)==4
  for st in old:m.remove(st)
  bar=int(m.get('n'))
  distribution=[(1,[0,1]),(2,[2,3])]
  for sn,vs in distribution:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2}[target.get('accid.ges')];before=state.get(key,0 if key[0] in '' else 0)
  ...
  for c in list(m):
   ...
   if c.tag==M+'tie':
    v=notes[c.get('startid')[1:]]['voice'];c.set('curvedir','below' if v in [1,3] else 'above')
 return root
...
def complete(is_keyboard):
 ...
 if is_keyboard:root=keyboard(root)
  ...
  for st in root.findall('.//m:staffDef',NS):
   v=int(st.get('n'))-1
   ET.SubElement(st,M+'label').text=['Soprano','Alto','Tenor','Bass'][v];ET.SubElement(st,M+'labelAbbr').text=['S','A','T','B'][v]
 ...
 if is_keyboard:
  # The final chord is shared by soprano and alto; show one fermata per stave.
  final=root.find('.//m:measure[@n="27"]',NS);seen=set()
  for fermata in final.findall(M+'fermata'):
   staff=fermata.get('staff')
   if staff in seen:final.remove(fermata)
   else:seen.add(staff)
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2500 if is_keyboard else 2700,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':846,'mnumInterval':1,'spacingStaff':16 if is_keyboard else 10})
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/62)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==786
 ...
 print(mode,': all 786 notes and 52 ties preserved.',flush=True)

def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,2,1,.5,6.25),('motif',2,2,1,5.25,6.25)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2200 if kind=='subject' else 1800,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':1846,'mnumInterval':1})
