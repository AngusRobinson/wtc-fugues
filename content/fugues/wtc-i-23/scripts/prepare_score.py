"""Parse the credited four-spine score, preserving closing chords and ties."""
...
raw=(root/'sources/bwv868.krn').read_text()
events=[];clocks=[0.,0.,0.,0.];bar=0
...
assert clocks==[136]*4 and bar==34
...
# The source keeps S/A on one stave; the analytical score separates all four.
print('Converting Humdrum',flush=True)
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':868})
assert tk.loadData(raw.replace('*staff2\t*staff2\t*staff1\t*staff1','*staff4\t*staff3\t*staff2\t*staff1'))
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in staff.findall('.//m:note',ns):
   ...
   alt=codes[sign] if sign else (sustained[(e['voice'],key)] if e['tied'] else state.get(key,1 if pname in 'fcgda' else 0))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/58)<2,(e,values)
(root/'sources/bwv868-four-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':136},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 34 bars; every pitch/onset agrees with Verovio.')
