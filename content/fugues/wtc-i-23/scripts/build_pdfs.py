...
for mode,suffix,description in [('open','annotated','Four staves'),('keyboard','keyboard','Two staves')]:
 ...
 for page,batch in enumerate(batches):
  ...
  if page==0:
   ...
   c.drawString(left,H-93,'Voice prefixes: S soprano, A alto, T tenor, B bass. Beats are crotchets.')
   y=H-105
  else:
   c.setFont('Times-Roman',11);c.drawString(left,H-29,'Bach - Fugue in B major, BWV 868')
