...
raw=(root/'sources/bwv878.krn').read_text()
...
for line_no,line in enumerate(raw.splitlines(),1):
 ...
 if line.startswith('='):
  ...
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*8]*4,(bar,clocks)
  continue
...
assert clocks==[344]*4 and bar==43
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':878})
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/124)<2,(e,values)
(root/'sources/bwv878-four-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':344},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 43 bars; every pitch/onset agrees with Verovio.')
