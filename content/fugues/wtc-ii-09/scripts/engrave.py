...
source=ET.parse(R/'sources/bwv878-four-voices.mei').getroot()
...
colours={'subject':'#244f91','cs1':'#a45112','cs2':'#843d7c','cs3':'#35735a','motif':'#637878'}
ranges=[(a,min(a+3,43)) for a in range(1,44,4)]
def serial(root):
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'4s','mode':'major','pname':'e'});ET.SubElement(st,M+'meterSig',{'count':'4','unit':'2'})
 for m in root.findall('.//m:measure',NS):
  old=m.findall(M+'staff');assert len(old)==4
  for st in old:m.remove(st)
  for sn,vs in [(1,[0,1]),(2,[2,3])]:
  ...
  for c in list(m):
   ...
   if c.get('staff'):c.set('staff','1' if int(c.get('staff'))<=2 else '2')
...
def complete(is_keyboard):
 ...
 for start,end in ranges:
  ...
  for ann in anns:
   ...
   if ann['start']<(start-1)*8 and ann['end']-(start-1)*8<=1:continue
   first=ns[0];v=ann['voice'];m=sec.find(f'm:measure[@n="{first["bar"]}"]',NS)
   staff=1 if v<2 else 2
   d=ET.SubElement(m,M+'dir',{ID:f'mark-{ann["id"]}-{start}','staff':str(staff if is_keyboard else v+1),'startid':'#'+first['id'],'place':'below' if is_keyboard and v%2==1 else 'above','color':colours[ann['kind']]})
   text=('SATB'[v]+': ' if is_keyboard else '')+ann['label']+(' >' if ann['start']<(start-1)*8 else '')
 ...
 assert len(root.findall('.//m:tie',NS))==len(source.findall('.//m:tie',NS))==52
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2700 if is_keyboard else 2900,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':878,'mnumInterval':1,'spacingStaff':16 if is_keyboard else 10})
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/124)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==802
 ...
 print(mode,': 11 systems, all 802 notes and 52 ties preserved.',flush=True)

def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,2,3,0,13),('cs1',3,4,3,16,30),('cs2',16,17,2,120,135),('cs3',16,17,3,120,136)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':1900,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':879,'mnumInterval':1})
 ...
 print('Four thematic examples.',flush=True)
