...
raw=(root/'sources/bwv854.krn').read_text()
...
assert clocks==[116.0]*len(clocks) and bar==29
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':854})
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in sorted(staff.findall('.//m:note',ns),key=lambda n:by_id[n.get(ID)]['start']):
   ...
   alt=codes[sign] if sign else (carried[(staff.get('n'),key)] if e['tied'] else state.get(key,{'f': 1, 'c': 1, 'g': 1, 'd': 1}.get(pname,0)))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/104)<2,(e,values)
(root/'sources/bwv854-3-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':116.0},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 29 bars; every pitch/onset agrees with Verovio.')
