...
raw=(root/'sources/bwv850.krn').read_text()
events=[];clocks=[0.0, 0.0, 0.0, 0.0];voice_map=[3, 2, 1, 0];bar=0
for line_no,line in enumerate(raw.splitlines(),1):
 ...
 if line.startswith('='):
  ...
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*4.0]*len(clocks),(bar,clocks)
  continue
...
assert clocks==[108.0]*len(clocks) and bar==27
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':850})
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  state={}
  for note in sorted(staff.findall('.//m:note',ns),key=lambda n:by_id[n.get(ID)]['start']):
   ...
   alt=codes[sign] if sign else (carried[(staff.get('n'),key)] if e['tied'] else state.get(key,{'f': 1, 'c': 1}.get(pname,0)))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/60)<2,(e,values)
(root/'sources/bwv850-4-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':108.0},ensure_ascii=False,separators=(',',':')))
