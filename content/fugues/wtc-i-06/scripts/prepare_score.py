...
raw=(root/'sources/bwv851.krn').read_text()
...
for line_no,line in enumerate(raw.splitlines(),1):
 ...
 if line.startswith('='):
  ...
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*3.0]*len(clocks),(bar,clocks)
  continue
...
assert clocks==[132.0]*len(clocks) and bar==44
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':851})
...
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in sorted(staff.findall('.//m:note',ns),key=lambda n:by_id[n.get(ID)]['start']):
   ...
   alt=codes[sign] if sign else (carried[(staff.get('n'),key)] if e['tied'] else state.get(key,{'b': -1}.get(pname,0)))
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/78)<2,(e,values)
(root/'sources/bwv851-3-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':132.0},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 44 bars; every pitch/onset agrees with Verovio.')
