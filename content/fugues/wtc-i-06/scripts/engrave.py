...
source=ET.parse(R/'sources/bwv851-3-voices.mei').getroot()
...
ranges=[(a,min(a+3,44)) for a in range(1,45,4)]
def serial(root):
...
LOW_ALTO=set(range(1,7))|set(range(13,28))|set(range(30,39))|{43,44}
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'1f','mode':'minor','pname':'d'});ET.SubElement(st,M+'meterSig',{'count':'3','unit':'4'})
  ...
  for sn in [1,2]:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2,'x':2}[target.get('accid.ges')];before=state.get(key,-1 if key[0]=='b' else 0)
...
def complete(is_keyboard):
 ...
 for measure in root.findall('.//m:measure',NS):
  ...
  for f in measure.findall(M+'fermata'):
   ...
   key=(f.get('staff'),min(notes[n.get(ID)]['start'] for n in ns))
   if key in seen:measure.remove(f)
   else:seen.add(key)
 mode='keyboard' if is_keyboard else 'open'
 (TEMP/f'{mode}.mei').write_text(serial(root))
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2300 if is_keyboard else 2500,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':851,'mnumInterval':1,'spacingStaff':12 if is_keyboard else 11})
 ...
 if not args.layout_only:
  ...
  for nid,e in notes.items():
   ...
   assert abs(actual['time']-e['start']*60000/78)<2,(mode,nid)
 systems=[];rendered=[]
...
def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,3,0,.5,7),('cs1',3,5,0,7,15)]:
  ...
  for m in list(sec):
   ...
   if kind=='subject' and int(m.get('n'))==b:
    layer=m.find('.//m:layer',NS);last=next(n for n in layer.iter(M+'note') if notes[n.get(ID)]['start']==(b-1)*3)
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':1900,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':851,'mnumInterval':1})
