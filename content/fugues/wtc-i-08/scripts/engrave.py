...
source=ET.parse(R/'sources/bwv853-3-voices.mei').getroot()
...
colours={'subject':'#244f91','augmentation':'#843d7c','motif':'#637878'}
ranges=[(a,min(a+3,87)) for a in range(1,88,4)]
def serial(root):
...
LOW_ALTO=set(range(19,24))|set(range(52,57))|set(range(81,88))
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'6s','mode':'minor','pname':'d'});ET.SubElement(st,M+'meterSig',{'count':'4','unit':'4'})
 for m in root.findall('.//m:measure',NS):
  ...
  for sn in [1,2]:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2,'x':2}[target.get('accid.ges')];before=state.get(key,1 if key[0] in 'fcgdae' else 0)
  ...
  for c in list(m):
   ...
   if c.tag==M+'tie':
    first=notes[c.get('startid')[1:]];last=notes[c.get('endid')[1:]]
    c.set('curvedir','below' if first['voice']==1 and staff_for(1,last['bar'])==1 else 'above')
 return root
...
def complete(is_keyboard):
 ...
 if is_keyboard:
  for tie in root.findall('.//m:tie',NS):
   first=notes[tie.get('startid')[1:]]
   if first['voice']==1 and first['start']==319:tie.set('curvedir','above')
 ...
 tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':2300 if is_keyboard else 2500,'pageHeight':20000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'encoded','xmlIdSeed':853,'mnumInterval':1,'spacingStaff':12 if is_keyboard else 11})
 ...
 if not args.layout_only:
  ...
  for nid,e in notes.items():
   ...
   assert abs(actual['time']-e['start']*60000/74)<2,(mode,nid)
 systems=[];rendered=[]
...
def examples():
 out=[]
 for kind,a,b,voice,start,end in [('subject',1,3,1,0,11.5),('augmentation',62,64,2,244,256)]:
  ...
  tk=verovio.toolkit();tk.setOptions({'inputFrom':'mei','pageWidth':1900,'pageHeight':5000,'scale':40,'adjustPageHeight':True,'header':'none','footer':'none','svgViewBox':True,'breaks':'none','xmlIdSeed':853,'mnumInterval':1})
 ...
 print('Subject and augmented-head examples.',flush=True)
