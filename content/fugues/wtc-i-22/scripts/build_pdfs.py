...
for mode,suffix in [('open','annotated'),('keyboard','keyboard')]:
 ...
 description='Two staves' if mode=='keyboard' else ('Four staves' if cfg['voices']==4 else 'Five staves')
 ...
 for page,batch in enumerate(batches):
  ...
  if page==0:
   ...
   legend=[(left,'S subject / A '+cfg['answer']+' answer','#244f91'),(left+230,'d / di / h* derived figures','#637878')]
   ...
   c.drawString(left,H-82,'* varied / incomplete; > continuation; d descending '+cfg['figure']+' figure; di its inversion; h* head allusion')
   y=H-98
  else:y=H-28
