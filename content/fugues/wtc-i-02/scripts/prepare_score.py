...
raw=(root/'sources/bwv847.krn').read_text()
events=[];clocks=[0.,0.,0.];bar=0
for line_no,line in enumerate(raw.splitlines(),1):
 if not line or line[0] in '! *':continue
 if line.startswith('='):
  match=re.match(r'=(\d+)',line)
  if match:
   bar=int(match[1]);assert clocks==[(bar-1)*4]*3,(bar,clocks)
  continue
 tokens=line.split('\t');assert len(tokens)==3
 for spine,token in enumerate(tokens):
  ...
  for j,part in enumerate(chord,1):
   ...
   events.append(dict(id=f'note-L{line_no}F{spine+1}'+(f'S{j}' if len(chord)>1 else ''),voice=2-spine,start=start,duration=duration,pitch=pitch,bar=bar,name=letter.upper()+'♭'*acc.count('-')+'♯'*acc.count('#')+str(octave),diatonic=octave*7+'cdefgab'.index(letter),token=part,tied=('_' in part or ']' in part)))
assert clocks==[124]*3 and bar==31
...
tk=verovio.toolkit();tk.setOptions({'inputFrom':'humdrum','xmlIdSeed':847})
...
by_id={e['id']:e for e in events};codes={'ff':-2,'f':-1,'n':0,'s':1,'ss':2}
for measure in tree.findall('.//m:measure',ns):
 for staff in measure.findall('m:staff',ns):
  ...
  for note in staff.findall('.//m:note',ns):
   ...
   alt=codes[sign] if sign else state.get(key,-1 if pname in 'bea' else 0)
...
for e in events:
 ...
 assert abs(values['time']-e['start']*60000/72)<2,(e,values)
(root/'sources/bwv847-three-voices.mei').write_text(mei)
(root/'data/score.json').write_text(json.dumps({'events':events,'audio':audio,'duration':124},ensure_ascii=False,separators=(',',':')))
print(f'{len(events)} written notes, {len(audio)} sounding events, 31 bars; every pitch/onset agrees with Verovio.')
