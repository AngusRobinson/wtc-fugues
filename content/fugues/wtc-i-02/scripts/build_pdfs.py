...
p=argparse.ArgumentParser();p.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[1]);p.add_argument('--prout',type=Path,required=True);args=p.parse_args();R=args.root
...
for mode,suffix,description in [('open','annotated','Annotated open score / Three staves'),('keyboard','keyboard','Annotated keyboard score / Two staves')]:
 ...
 for page in range(2):
  ...
  if page==0:
   ...
   c.drawString(left,H-93,'Voice prefixes: S soprano, A alto, B bass. Counterpoint boundaries follow Keller.')
   y=H-102
  else:
   c.setFont('Times-Roman',11);c.drawString(left,H-29,'Bach - Fugue in C minor, BWV 847')
   c.setFont('Helvetica',8);c.drawRightString(W-left,H-29,f'Bars {batch[0]["start"]}-{batch[-1]["end"]}')
   y=H-42
  heights=[width*float(re.search(r'viewBox="[\d.]+ [\d.]+ ([\d.]+) ([\d.]+)"',s['svg'])[2])/float(re.search(r'viewBox="[\d.]+ [\d.]+ ([\d.]+) ([\d.]+)"',s['svg'])[1]) for s in batch]
  gap=min(26,(y-44-sum(heights))/3);assert gap>=8,(mode,page,gap,heights)
  for s,h in zip(batch,heights):
   c.drawImage(str(TEMP/f'{mode}-{s["start"]}.png'),left,y-h,width=width,height=h);y-=h+gap
  ...
  c.drawRightString(W-left,23,f'{page+1} / 2');c.showPage()
