...
source=ET.parse(R/'sources/bwv847-three-voices.mei').getroot()
...
ranges=[(a,min(a+3,31)) for a in range(1,32,4)]
def serial(root):
...
def keyboard(root):
 ...
 for sn,clef,line in [(1,'G','2'),(2,'F','4')]:
  ...
  ET.SubElement(st,M+'clef',{'shape':clef,'line':line});ET.SubElement(st,M+'keySig',{'sig':'3f','mode':'minor','pname':'c'});ET.SubElement(st,M+'meterSig',{'count':'4','unit':'4'})
 for m in root.findall('.//m:measure',NS):
  old=m.findall(M+'staff');assert len(old)==3
  for st in old:m.remove(st)
  for sn,vs in [(1,[0,1]),(2,[2])]:
   ...
   for q,ns in sorted(bytime.items()):
    ...
    for note in ns:
     ...
     alt={'ff':-2,'f':-1,'n':0,'s':1,'ss':2}[target.get('accid.ges')];before=state.get(key,-1 if key[0] in 'bea' else 0)
...
def complete(is_keyboard):
 ...
 for nid,e in notes.items():
  ...
  assert abs(actual['time']-e['start']*60000/72)<2,(mode,nid)
 systems=[];rendered=[]
 ...
 assert len(rendered)==len(set(rendered))==770
 ...
 print(mode,': 8 systems, all 770 notes and 16 ties preserved.',flush=True)
